...
export default function RoomBottombar(props) {
  ...
  return (
    <BottomNavigation
      ...
      style={{width: isDesktop() ? 450 : '100%', height: 72, transform: isDesktop() ? 'rotate(90deg)' : undefined, zIndex: 2499, position: 'fixed', bottom: (isDesktop() && isInRoom()) ? (currentRoomNavBackup === 2 && isConfConnected) ? 'calc(50% + 112px)' : '50%' : bottom, left: isDesktop() ? -160 : undefined, borderRadius: isDesktop() ? 32 : 0, transition: 'bottom .5s', backgroundColor: colors.primaryMedium, backdropFilter: 'blur(10px)'}}
    >
