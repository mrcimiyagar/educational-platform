...
import {humanFileSize, socket, useForceUpdate} from "../../util/Utils";
            ...
            <div style={{height: 'auto'}}>
                    ...
                    return (
                        <Card style={{backgroundColor: colors.primaryLight, width: 'calc(100% - 32px)', marginTop: 8, marginLeft: 16, marginRight: 16, height: 'auto', direction: 'rtl'}} onClick={(e) => {
                            ...
                            <a href={`../file/download_file?token=${token}&roomId=${props.roomId}&fileId=${file.id}`} download={file.name}>
                            <div style={{display: 'flex'}}>
                                {
                                    <img key={index} alt="Thumbnail" src={file.local ? file.src : `../file/download_file?token=${token}&roomId=${props.roomId}&fileId=${file.previewFileId}`} className="list-thumbnail responsive border-0" />
                                }
                                <div>
