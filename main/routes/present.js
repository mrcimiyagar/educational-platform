                ...
                form.parse(req, async function (err, fields, files) {
                    ...
                    if (ext === 'pdf') {
                        ...
                        try {
                            ...
                            let newPath = rootPath + '/files/' + file.id;
                            let previewFactoryPath = rootPath + '/temp/' + file.id + '_' + '.pdf';
...
router.post('/get_presents', jsonParser, async function (req, res) {
    authenticateMember(req, res, async (membership, session, user) => {
            ...
            sw.File.findAll({where: {roomId: membership.roomId, isPreview: false, isPresent: true}}).then(files => {
                sw.Present.findAll({where: {fileId: files.map(f => f.id)}}).then(async function (presents) {
...
router.post('/pick_present', jsonParser, async function (req, res) {
    authenticateMember(req, res, async (membership, session, user) => {
            ...
            sw.Present.findOne({where: {roomId: membership.roomId, id: req.body.presentId}}).then(async function (present) {
                if (present === null) {
                    res.send({status: 'error', errorCode: 'e0005', message: 'presentation does not exist.'});
                    return;
                }
                sw.RoomSecret.findOne({where: {roomId: membership.roomId}}).then(async function (room) {
                    room.presentId = present.id;
                    room.save();
                    require("../server").pushTo('room_' + membership.roomId, 'present-picked', present);
                    ...
                    try {
                        ...
                        for (let i = 0; i < 100; i++) {
                            if (fs.existsSync(rootPath + '/pdfPages/' + fileId + '.' + i + '.png')) {
                                fs.unlinkSync(rootPath + '/pdfPages/' + fileId + '.' + i + '.png');
                            }
                        }
                        ...
                        convert(present.pageNumber, true).then((output) => {
                          console.log('converted successfully.');
                            require("../server").pushTo('room_' + membership.roomId, 'page-switched', {pn: present.pageNumber, image: output.base64});
...
router.post('/swich_page', jsonParser, async function (req, res) {
    authenticateMember(req, res, async (membership, session, user) => {
            ...
            sw.RoomSecret.findOne({where: {roomId: membership.roomId}}).then(async function (room) {
                ...
                sw.Present.findOne({where: {id: room.presentId}}).then(present => {
                    ...
                    try {
                        ...
                        convert(req.body.pageNumber, true).then((output) => {
                            console.log('converted successfully.');
                            require("../server").pushTo('room_' + membership.roomId, 'page-switched', {pn: req.body.pageNumber, image: output.base64});
                            res.send({status: 'success'});
                        });
                    }
                    catch(ex) {console.error(ex);}
            ...
            sw.RoomSecret.findOne({where: {roomId: membership.roomId}}).then(async function (room) {
                ...
                sw.Present.findOne({where: {id: room.presentId}}).then(present => {
                    ...
                    try {
                        ...
                        convert(present.pageNumber, true).then((output) => {
                            console.log('converted successfully.');
                            res.send({status: 'success', currentPage: output.base64, pageNumber: present.pageNumber});
                        });
                    }
                    catch(ex) {console.error(ex);}
                })
