...
const {removeUser, getRoomUsers, addUser, getGuestUser, guestAccsByUserId} = require('./users');
...
let disconnectWebsocket = (user) => {
    ...
    models.Room.findOne({where: {id: roomId}}).then(room => {
        ...
        if (room !== null) {
            models.Room.findAll({raw: true, where: {spaceId: room.spaceId}}).then(async rooms => {
                for (let i = 0; i < rooms.length; i++) {
                    let room = rooms[i];
                    removeUser(room.id, user.id)
                    room.users = getRoomUsers(room.id)
                }
                let mem = await models.Membership.findOne({where: {roomId: req.body.roomId, userId: session.userId}})
                if (membership === null || membership === undefined) {
                    if (guestAccsByUserId[session.userId] === undefined || guestAccsByUserId[session.userId] === null) {
                        res.send({status: 'error', errorCode: 'e0005', message: 'membership does not exist.'})
                        return
                    }
                    mem = guestAccsByUserId[session.userId]
                }
                require("./server").pushTo('room_' + sockets[user.id].roomId, 'user-exited', {rooms: rooms, users: getRoomUsers(mem.roomId)})
            })
        }
    })
}
        ...
        wss.on('connection', (ws) => {
            ...
            try {
                ...
                soc.on('auth', ({token}) => {
                    ...
                    try {
                        models.Session.findOne({where: {token: token}}).then(async function (session) {
                            if (session == null) {
                                ...
                                if (user !== null) {
                                    ...
                                    soc.on('chat-typing', roomId => {
                                        models.Membership.findOne({where: {userId: user.id, roomId: roomId}}).then(mem => {
                                            if (mem !== null) {
                                                if (soc.room !== null && soc.room !== undefined) {
                                                    if (soc.user.id in soc.room.typing) {
                                                        clearTimeout(soc.room.typing[soc.user.id].timeout);
